...
class dbapi {
	...
	public function getStops($route) {
		...
		if (self::testForError($result)) {
			header("Content-Type: application/json",true);
			echo json_encode(array('route' => $route, 'stops' =>$result['GetStopDataByRouteResult']['diffgram']['StopDataByRoute']), JSON_PRETTY_PRINT);
		}
	}
	...
	public function getStopTimes($stop) {
		...
		if (self::testForError($result)) {
			...
			if (isset($result['GetRealTimeStopDataResult']['diffgram']['DocumentElement'])) {
				echo json_encode(array('stopId' => $stop, 'departures' => $result['GetRealTimeStopDataResult']['diffgram']['DocumentElement']['StopData']), JSON_PRETTY_PRINT);
			} else {
